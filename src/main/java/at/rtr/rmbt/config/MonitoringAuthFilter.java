 ...
 *
 * <p>This server has no Spring Security, so the report would otherwise be world-readable. The
 * report is protected here by a password taken from the deployment context: the {@code MELODY_PW}
 * value of a Tomcat {@code conf/context.xml} {@code <Parameter>} (falling back to a JVM system
 * property / environment variable). Only the password is checked - the Basic-auth username may be
 * anything.
 *
